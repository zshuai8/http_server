 ...
 * <div id="loadavg"> </div>
 *
 * This code is written in a manner that should not interfere with
 * the original page in any way.
 *
...
// (code mirrors $.getScript - but we don't have jQuery yet)
...
/**
 * A Barrier object invokes a callback passed to the constructor
...
function dowithJQuery($) {
    ...
    // amazingly, JS does not have a function to resolve relative URLs
            ...
            function update () {
                $.getJSON(url + "?callback=?", function (data) {
                    ...
                    plot = showMemory(plot, $div.attr('id'),
                        ...
                        data.MemTotal / MB, resolveURL(url)
                    );
                });
            }
            update ();
    ...
    // start loading required scripts asynchronously, execute a barrier
    // when they have all loaded.
    ...
    if ($.browser.msie)
        $.getScript("js/excanvas.js", barrier.add());

    // the jqplot* scripts require jquery.jqplot.js, so delay loading
    // them until jqplot has loaded.
    var jqplotloaded = barrier.add();
    $.getScript("js/jquery.jqplot.js", function () {
        $.each([ "js/jqplot/jqplot.barRenderer.js", 
                 "js/jqplot/jqplot.categoryAxisRenderer.js"
               ],
               function (index, jsfile) {
                    $.getScript(jsfile, barrier.add());
               });

        jqplotloaded();
    });

    $(document).ready(barrier.add());
    barrier.ready();
}
